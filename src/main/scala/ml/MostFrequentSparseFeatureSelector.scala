...
import org.apache.spark.ml.param.{ParamMap, HasInputCol}
 ...
 */
class MostFrequentSparseFeatureSelector(numFeatures: Int)
    extends Estimator[SparseFeatureVectorizer] with HasInputCol {
  /**
  ...
  override def fit(dataset: SchemaRDD, paramMap: ParamMap): SparseFeatureVectorizer = {
    ...
    val featureSpace = instances.flatMap(_.map(_._1)).countByValue().toSeq.sortBy(-_._2)
        .take(numFeatures).map(_._1).zipWithIndex.toMap
